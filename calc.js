...
// ===============================
// Configuration (explicit + conservative)
// ===============================
const INFLATION_RATE = 0.03;      // 3% inflation
const GROWTH_RATE = 0.08;         // 8% conservative normalized tech growth
...
function runCalculation() {
  const evToday = parseFloat(document.getElementById("evToday").value);
  const years = parseInt(document.getElementById("exitYears").value);

  if (isNaN(evToday) || isNaN(years)) {
    alert("Please enter valid numbers.");
    return;
  }

  const evInflation =
    evToday * Math.pow(1 + INFLATION_RATE, years);

  const evGrowth =
    evToday * Math.pow(1 + INFLATION_RATE + GROWTH_RATE, years);

  document.getElementById("evInflation").textContent = formatUSD(evInflation);
  document.getElementById("evGrowth").textContent = formatUSD(evGrowth);

  renderChart(evToday, evInflation, evGrowth);
}

// ===============================
// Chart Rendering (bar chart only)
// ===============================
function renderChart(evToday, evInflation, evGrowth) {
  ...
  chartInstance = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        data: [evToday, evInflation, evGrowth],
        backgroundColor: [
          "#4E79A7",
          "#59A14F",
          "#9C755F"
